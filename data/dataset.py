...
class CompositionDataset(Dataset):
    ...
    def __init__(
        ...
        #修改
        # 打开文件，使用 'w' 模式表示写入
        with open('selfdata.txt', 'w') as f:
            # 遍历列表中的每个元素，将其转换为字符串并写入文件中
        ...
        if not self.update_features:
            ...
            #修改1
            # list1=activation_data['files']
            # list2=[path.replace('\\', '/') for path in list1]
            # 打开文件，使用 'w' 模式表示写入
            with open('activation_data.txt', 'w') as f:
                # 遍历列表中的每个元素，将其转换为字符串并写入文件中
                for item in activation_data['files']:
                    f.write(str(item) + '\n')
